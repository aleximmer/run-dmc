...
def getEncodedSplit(data: pd.DataFrame):	
	...
	X, Y = dmc.transformation.transform(data, scaler=dmc.transformation.scale_features, binary_target=True)
...
def prepareFeatureList(df: pd.DataFrame,  dont_use) -> np.array:
	return np.array([f for f in df.columns.tolist() if not f in dont_use])	


def playGod(name, split, classifier, data, features):
	...
	for j in range(1, generations+1):
		...
		print("classifier: " + name + " test: " + split + " generation: " + str(j) + " top-score: " + str(np.max(currentFitness)) + " top-size: " + str(np.sum(genes[0])))	
	return (featureSetTable, featureScoreTable) 	
